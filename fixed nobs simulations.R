...
#          grads <- bilinearGradVec(cbind(x_samples[j, ], y_samples[j, ]), covlist)
#          us <- (delta*par[4]/((N+1)*2)) * (par[1] * grads[1,,] + par[2] * grads[2,,] + par[3] * grads[3,,]) 
#          us <- rbind(u_0, us)
#          
#          g = cbind(grad_0[,1,],array(aperm(grads, c(1, 3, 2)), dim = c(3, 2 * N)))
#
#          D = (cbind(full_x[j,0:N+2], full_y[j,0:N+2]) - cbind(full_x[j,0:N+1], full_y[j,0:N+1])) - us
#
#          
#          rbind(prod(dmvn(D, matrix(c(0,0)), diag(delta*par[4]/(N+1), 2, 2)))*L_k[j] ,
#                g%*%matrix(t(D), ncol = 1), 
#                -(N+1)/par[4] + (N+1)/(2*delta*par[4]^2)*t(matrix(t(D), ncol = 1))%*%matrix(t(D), ncol = 1) + (1/(2*par[4]))* t(t(g)%*%par[1:3]) %*% matrix(t(D), ncol = 1))
#          
#        })
#        L_k = lik_grad_k[1, ]
#        return(c(log(sum(L_k/M)), -sum(lik_grad_k[2, ]*L_k)/(2*sum(L_k)), -sum(lik_grad_k[3, ]*L_k)/(2*sum(L_k)), -sum(lik_grad_k[4, ]*L_k)/(2*sum(L_k)), -sum(lik_grad_k[5, ]*L_k)/(sum(L_k))))
#      }
      ...
      compute <- function(i){
        ...
        L_k = P[i, 1:M]*gamma^(2*N)
          #1 / (mvnfast::dmvn(x_samples, mu_x, sigma = chol_matrix, isChol = TRUE) *
          #           mvnfast::dmvn(y_samples, mu_y, sigma = chol_matrix, isChol = TRUE))
        
        grad_0 <- NULL # not needed in R any more (computed inside C++)
